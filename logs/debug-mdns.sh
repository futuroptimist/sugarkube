...
sudo systemctl status avahi-daemon --no-pager 2>&1 | sanitize_ip_mac

print_section "Check if mDNS port is listening"
# This only shows 0.0.0.0:5353, which is not sensitive
if command -v netstat >/dev/null 2>&1; then
  sudo netstat -ulnp 2>/dev/null | grep 5353 | sanitize_ip_mac || \
    echo "No process listening on UDP 5353 via netstat"
elif command -v ss >/dev/null 2>&1; then
  sudo ss -ulnp 2>/dev/null | grep 5353 | sanitize_ip_mac || \
    echo "No process listening on UDP 5353 via ss"
else
...
if command -v avahi-browse >/dev/null 2>&1; then
  timeout 5 avahi-browse -t -r _k3s-sugar-dev._tcp 2>/dev/null | \
    filter_avahi_output | sanitize_ip_mac || \
...
if command -v iptables >/dev/null 2>&1; then
  sudo iptables -L -n -v 2>/dev/null | grep -E "5353|mdns|multicast" | sanitize_ip_mac || \
