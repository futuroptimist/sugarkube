...
def test_configmap_patch_strips_credentials_file(cf_recipe_body: str) -> None:
    ...
    config_yaml = match.group("body").strip()
    expected = """
    apiVersion: v1
    kind: ConfigMap
    metadata:
    ...
    assert config_yaml == expected
    assert "credentials-file" not in config_yaml
    assert "creds" not in config_yaml
...
def test_deployment_patch_enforces_token_mode(deployment_patch_ops: list[dict]) -> None:
    ...
    expected_args = [
        (
            "exec cloudflared tunnel --config /etc/cloudflared/config/config.yaml "
