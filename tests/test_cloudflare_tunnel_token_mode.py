...
def test_deployment_patch_enforces_token_mode(deployment_patch_ops: list[dict]) -> None:
    ...
    env_from_op = ops_by_path.get("/spec/template/spec/containers/0/envFrom")
    assert env_from_op and env_from_op.get("op") in {"add", "replace"}
    assert env_from_op.get("value") == []
...
def test_cf_tunnel_install_flags_origin_cert_logs(cf_recipe_body: str) -> None:
    ...
    assert "client didn't specify origincert path" in cf_recipe_body
    assert "origin-cert / credentials.json flow" in cf_recipe_body
    assert "Confirm the tunnel is remote-managed" in cf_recipe_body
...
def test_reset_and_debug_recipes_exist_and_reset_is_safe() -> None:
    ...
    assert "configuration is injected via env/command" in debug_body
