...
def _write_required_stls(stl_dir: Path, *, include_variants: bool = True) -> None:
    ...
    for name in [
        stack_dir / "carriers" / "pi_carrier_stack_mounts_printed.stl",
        stack_dir / "carriers" / "pi_carrier_stack_mounts_heatset.stl",
...
def test_package_stl_artifacts_groups_files(tmp_path: Path) -> None:
    ...
    assert {path.name for path in (stack_root / "carriers").iterdir()} == {
        "pi_carrier_stack_mounts_printed.stl",
        "pi_carrier_stack_mounts_heatset.stl",
    }
    ...
    assert "- carriers/\n  - pi_carrier_stack_mounts_printed.stl" in readme
