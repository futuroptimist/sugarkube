...
def test_run_without_apply_prompts_followup():
    system = FakeSystem(taps=[], packages=set())
    buffer = io.StringIO()
    wizard = MODULE.SetupWizard(system, stream=buffer)
    wizard.run(force=False, apply=False)
    assert "Re-run with --apply" in buffer.getvalue()


def test_run_with_empty_plan_reports_completion(monkeypatch):
    system = FakeSystem()
    buffer = io.StringIO()
    wizard = MODULE.SetupWizard(system, stream=buffer)
    monkeypatch.setattr(wizard, "build_plan", lambda force: [])
    wizard.run(force=False, apply=False)
    text = buffer.getvalue()
    assert "prerequisites are already satisfied" in text
    assert text.strip().endswith("macOS setup complete.")
