...
def test_handles_combined_checksum_files(tmp_path):
    env = write_stub_scripts(tmp_path)
    sha_path = Path(env["SHA_SOURCE"])
    target_digest = hashlib.sha256(Path(env["IMAGE_SOURCE"]).read_bytes()).hexdigest()
    sha_path.write_text(
        "".join(
            (
                f"{'0' * 64}  other.img.xz\n",
                f"{target_digest}  sugarkube.img.xz\n",
                f"{'f' * 64}  extras/ignored.img\n",
            )
        )
    )

    out = tmp_path / "combined" / "sugarkube.img.xz"
    result = subprocess.run(
        ["/bin/bash", str(download_script_path()), "-o", str(out)],
        env=env,
        cwd=tmp_path,
    ...
    assert sha_file.read_text() == f"{target_digest}  sugarkube.img.xz\n"
