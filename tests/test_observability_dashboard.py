...
def test_daniel_visitor_dashboard_contract_is_bounded_scoped_and_fail_closed(dashboards):
    for document in (json.loads(TEMPLATE.read_text()), *dashboards):
        ...
        assert "monitoring_enabled" in success
        assert 'state=~"success|recovered"' in success
        assert "<= 1020" in success
        unavailable = validator.DANIEL_VISITOR_PANEL_CONTRACT[
            "Daniel visitor journey unavailable or stale"
        ][0]
        assert 'state=~"disabled|unavailable|stale"' in unavailable
        assert (
            validator.DANIEL_VISITOR_PANEL_CONTRACT["Daniel visitor journey failure stage"][1]
            == "short"
        )


@pytest.mark.parametrize("title", validator.DANIEL_VISITOR_PANEL_CONTRACT)
def test_daniel_visitor_validator_rejects_contract_regressions(tmp_path, dashboards, title):
    changed = copy.deepcopy(dashboards[0])
    panel(changed, title)["targets"][0]["expr"] += " or vector(0)"
    with pytest.raises(SystemExit, match="Daniel visitor contract"):
...
def test_canonical_order_ids_grid_and_defaults(dashboards):
    ...
    visitor = staging["panels"][-7:]
    assert [item["id"] for item in visitor] == list(range(71, 78))
    assert [item["gridPos"] for item in visitor] == [
        {"h": 1, "w": 24, "x": 0, "y": 214},
        {"h": 8, "w": 12, "x": 0, "y": 215},
        {"h": 8, "w": 12, "x": 12, "y": 215},
        {"h": 8, "w": 12, "x": 0, "y": 223},
