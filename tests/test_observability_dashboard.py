...
def test_generator_check_and_outputs_are_deterministic(dashboards):
    ...
    assert len(staging["panels"]) == 71
    assert sum(item["type"] == "row" for item in staging["panels"]) == 12
    assert sum(item["type"] != "row" for item in staging["panels"]) == 59
...
def test_canonical_order_ids_grid_and_defaults(dashboards):
    ...
    assert rows == [
        ...
        "danielsmith.io essential visitor journey",
    ]
    assert [item["id"] for item in staging["panels"]] == list(range(1, 72))
