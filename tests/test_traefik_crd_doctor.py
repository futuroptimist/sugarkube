...
def test_doctor_reports_healthy_traefik_owned_crds(tmp_path: pathlib.Path) -> None:
    ...
    assert "you can safely run 'just traefik-install'" in result.stdout
    assert "Recommended actions" not in result.stdout
    assert not log_path.exists() or log_path.read_text() == ""
...
def test_doctor_reports_problematic_crds(tmp_path: pathlib.Path) -> None:
    ...
    assert "Detected problematic Gateway API CRDs" in result.stdout
...
def test_doctor_allows_unmanaged_crds(tmp_path: pathlib.Path) -> None:
    ...
    assert "Some CRDs are present without Helm ownership metadata" in result.stdout
    assert "run 'just traefik-install' to adopt" in result.stdout
    assert "Recommended actions" not in result.stdout
