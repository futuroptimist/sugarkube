...
def test_run_verifier_propagates_env_and_no_sudo(monkeypatch):
    args = SimpleNamespace(
        verifier_path="/bin/verifier",
        no_sudo=True,
        command_timeout=30,
    ...
    assert result.success
    assert "DSPACE_HEALTH_URL=https://dspace.example/status" in captured["command"]
...
def test_trigger_reboot_errors_on_nonzero(monkeypatch):
    args = SimpleNamespace(connect_timeout=1, no_sudo=False)
...
def test_format_summary_reports_failure():
    result = MODULE.SmokeTestResult(
        host="pi.local",
        checks=[{"name": "ping", "status": "fail"}],
        passes=0,
        total=1,
        failures=[{"name": "ping", "status": "fail"}],
    )
    summary = MODULE.format_summary(result)
    assert summary.startswith("[pi.local] FAIL")
