...
def test_parse_verifier_output_requires_normalized_checks():
    payload = {"checks": ["unexpected", 42, None]}
    with pytest.raises(MODULE.SmokeTestError):
...
def test_run_verifier_injects_environment(monkeypatch):
    args = SimpleNamespace(
        verifier_path="/bin/verifier",
        no_sudo=True,
        command_timeout=30,
        token_place_url=None,
        skip_token_place=True,
        dspace_url="https://dspace/health",
        skip_dspace=False,
    )
    ...
    assert "TOKEN_PLACE_HEALTH_URL=skip" in captured_command["value"]
    assert "DSPACE_HEALTH_URL=https://dspace/health" in captured_command["value"]
    assert "sudo" not in captured_command["value"]
...
def test_wait_for_ssh_handles_timeout_exception(monkeypatch):
    args = SimpleNamespace(connect_timeout=1, poll_interval=0)
    attempts = []
...
def test_main_records_failed_verifier(monkeypatch, capsys):
    host = "pi.local"
    summary = MODULE.SmokeTestResult(
        host=host,
        checks=[{"name": "c1", "status": "fail"}],
        passes=0,
        total=1,
        failures=[{"name": "c1", "status": "fail"}],
    )

    monkeypatch.setattr(MODULE, "run_verifier", lambda *_: summary)
    ...
    start = captured.out.index("{")
    payload = json.loads(captured.out[start:])
    assert payload["results"][0]["failures"][0]["name"] == "c1"
