...
def test_build_env_prefers_explicit_urls_over_defaults():
    args = SimpleNamespace(
...
def test_run_verifier_times_out(monkeypatch):
    ...
    def fake_run_ssh(host, parsed_args, remote_command, timeout):
        raise MODULE.subprocess.TimeoutExpired(cmd=remote_command, timeout=timeout)
...
def test_trigger_reboot_raises_for_unexpected_code(monkeypatch):
    args = SimpleNamespace(connect_timeout=1, no_sudo=False)
    failure = CompletedProcess(
        args=[],
        returncode=2,
...
def test_main_records_failed_checks(monkeypatch, capsys):
    ...
    failing = MODULE.SmokeTestResult(
        host=host,
        checks=[{"name": "c1", "status": "fail", "details": "oops"}],
        passes=0,
        total=1,
        failures=[{"name": "c1", "status": "fail", "details": "oops"}],
    )
