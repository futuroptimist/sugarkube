...
def test_run_verifier_includes_env_overrides(monkeypatch):
    ...
    def fake_run_ssh(host, parsed_args, command, timeout):
        ...
        return CompletedProcess(
            args=["ssh"],
            returncode=0,
            stdout=json.dumps({"checks": [{"name": "env", "status": "pass"}]}),
            stderr="",
        )
    ...
    assert result.success
    assert result.total == 1
    assert observed["command"].startswith("TOKEN_PLACE_HEALTH_URL=")
    assert "DSPACE_HEALTH_URL=skip" in observed["command"]
    assert observed["command"].endswith("/bin/verifier --json --no-log")
