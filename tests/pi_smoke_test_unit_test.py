...
def test_parse_verifier_output_requires_non_empty_checks():
    payload = {"checks": ["unexpected"]}
    with pytest.raises(MODULE.SmokeTestError):
...
def test_main_collects_post_reboot_failures(monkeypatch, capsys):
    host = "pi.local"
    first = MODULE.SmokeTestResult(host=host, checks=[], passes=1, total=1, failures=[])
    post = MODULE.SmokeTestResult(
        host=host,
        checks=[{"name": "svc", "status": "fail"}],
        passes=0,
        total=1,
    ...
    assert payload["results"][1]["failures"] == post.failures
