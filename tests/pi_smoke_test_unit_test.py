...
def test_parse_verifier_output_requires_normalised_checks():
    payload = {"checks": ["nope", 17]}

    with pytest.raises(MODULE.SmokeTestError) as excinfo:
        MODULE.parse_verifier_output(json.dumps(payload))

    assert "did not contain any checks" in str(excinfo.value)
...
def test_main_records_post_reboot_failures(monkeypatch, capsys):
    ...
    monkeypatch.setattr(MODULE, "run_verifier", fake_run_verifier)
    monkeypatch.setattr(MODULE, "trigger_reboot", lambda *_: None)
    ...
    start = captured.out.index("{")
    payload = json.loads(captured.out[start:])
    post_reboot = payload["results"][1]
    assert post_reboot["phase"] == "post-reboot"
    assert post_reboot["failures"] == [{"name": "svc", "status": "fail"}]
