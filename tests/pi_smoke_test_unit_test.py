...
def test_parse_verifier_output_errors_when_no_valid_checks():
    payload = {"checks": ["bad", 42, []]}
    with pytest.raises(MODULE.SmokeTestError):
...
def test_run_verifier_includes_env_overrides(monkeypatch):
    args = SimpleNamespace(
        verifier_path="/bin/verifier",
        no_sudo=True,
        command_timeout=30,
    ...
    def fake_run_ssh(host, parsed_args, command, timeout):
        captured_command["remote"] = command
    ...
    monkeypatch.setattr(MODULE, "run_ssh", fake_run_ssh)
    result = MODULE.run_verifier("pi.local", args)
    assert "TOKEN_PLACE_HEALTH_URL" in captured_command["remote"]
    assert "DSPACE_HEALTH_URL" in captured_command["remote"]
    assert result.success
...
def test_wait_for_ssh_retries_after_timeout(monkeypatch):
    args = SimpleNamespace(connect_timeout=1, poll_interval=0)
    attempts = {"count": 0}
...
def test_trigger_reboot_accepts_zero(monkeypatch):
    args = SimpleNamespace(connect_timeout=1, no_sudo=True)

    monkeypatch.setattr(
        MODULE,
        "run_ssh",
        lambda *_, **__: CompletedProcess(args=[], returncode=0, stdout="", stderr=""),
    )
