...
def test_parse_verifier_output_skips_non_dict_entries():
    payload = {"checks": ["bogus", {"name": "real", "status": "pass"}]}
    output = json.dumps(payload)
    checks = MODULE.parse_verifier_output(output)
    assert checks == [{"name": "real", "status": "pass"}]
...
def test_run_verifier_timeout(monkeypatch):
    ...
    def fake_run_ssh(*_, **__):
        raise MODULE.subprocess.TimeoutExpired(cmd=["ssh"], timeout=30)
...
def test_trigger_reboot_raises_on_unexpected_code(monkeypatch):
    args = SimpleNamespace(connect_timeout=1, no_sudo=True)
    completed = CompletedProcess(args=[], returncode=12, stdout="", stderr="denied")
...
def test_main_handles_post_reboot_failure(monkeypatch, capsys):
    ...
    def fake_run_verifier(requested_host, args):
        if fake_run_verifier.second_call:
            raise MODULE.SmokeTestError("post reboot down")
        fake_run_verifier.second_call = True
        return initial

    fake_run_verifier.second_call = False
    ...
    assert payload["results"][1]["error"] == "post reboot down"
...
def test_format_summary_reports_counts():
    result = MODULE.SmokeTestResult(
        ...
        passes=1,
        total=2,
        failures=[{"name": "two", "status": "fail"}],
    )
    summary = MODULE.format_summary(result)
    assert "pi.local" in summary and "1/2" in summary and "1 failing" in summary
