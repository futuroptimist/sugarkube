...
def _setup_build_env(tmp_path, check_compose: bool = False):
    ...
    # Fake pi-gen repo that writes a zipped image
    git_stub = (
        f"#!/bin/bash\n"
        f'echo "$@" > "{git_log}"\n'
        "target=${!#}\n"
        ...
        "with zipfile.ZipFile('deploy/sugarkube.zip','w') as zf:\n"
    ...
    env["MV_LOG"] = str(tmp_path / "mv.log")
    return env
...
def test_uses_default_pi_gen_branch(tmp_path):
    ...
    assert not mv_log.exists() or "deploy" not in mv_log.read_text()
