...
def test_preinstalls_required_packages():
    ...
    required_packages = [
        ...
        "dbus",
        "libnss-mdns",
        "libglib2.0-bin",  # Provides gdbus for D-Bus checks
        "python3",
        "tcpdump",
        "nftables",
    ]
    ...
    for line_num, line in enumerate(script_text.split('\n'), 1):
        if 'ensure_packages' in line and '01-sys-tweaks/00-packages' in line:
            # Get the multi-line ensure_packages call
            start = line_num - 1
    ...
    for package in required_packages:
        assert package in packages_section, \
            f"Package '{package}' must be pre-installed in image build (found in: {packages_section[:200]}...)"


def test_libglib2_provides_gdbus():
    """Verify libglib2.0-bin package provides gdbus command for D-Bus checks"""
    # This is a documentation test to ensure we understand why libglib2.0-bin is required
