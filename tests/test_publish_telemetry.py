...
def test_invalid_timeout_env_raises_telemetry_error(monkeypatch):
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_TIMEOUT", "abc")
    monkeypatch.delenv("SUGARKUBE_TELEMETRY_VERIFIER_TIMEOUT", raising=False)
    with pytest.raises(MODULE.TelemetryError) as excinfo:
        MODULE.parse_args([])
    assert "invalid timeout value" in str(excinfo.value)
