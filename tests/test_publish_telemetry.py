...
def test_read_uptime_handles_blank(monkeypatch):
    monkeypatch.setattr(MODULE, "read_text", lambda path: "")
    assert MODULE.read_uptime() is None
...
def test_discover_verifier_path_skips_empty_candidates():
    class FlakyCandidate(str):
        def __new__(cls, value: str):
            obj = super().__new__(cls, value)
            obj._first = True  # type: ignore[attr-defined]
            return obj

        def __len__(self) -> int:
            if getattr(self, "_first", False):
                setattr(self, "_first", False)
                return 1
            return 0

    result = MODULE.discover_verifier_path(explicit=FlakyCandidate("ignored"))
    assert result is not None
    assert result.endswith("pi_node_verifier.sh")
