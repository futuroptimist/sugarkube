...
def test_parse_args_rejects_invalid_timeout(monkeypatch):
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_TIMEOUT", "not-a-number")
    with pytest.raises(MODULE.TelemetryError) as exc:
        MODULE.parse_args([])
    assert "--timeout" in str(exc.value)


def test_parse_args_uses_env_timeout(monkeypatch):
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_TIMEOUT", "15.5")
    args = MODULE.parse_args([])
    assert args.timeout == pytest.approx(15.5)


def test_parse_args_rejects_invalid_verifier_timeout(monkeypatch):
    monkeypatch.delenv("SUGARKUBE_TELEMETRY_TIMEOUT", raising=False)
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_VERIFIER_TIMEOUT", "oops")
    with pytest.raises(MODULE.TelemetryError) as exc:
        MODULE.parse_args([])
    assert "--verifier-timeout" in str(exc.value)
