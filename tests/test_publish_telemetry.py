...
def test_parse_args_invalid_timeout_env(monkeypatch):
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_TIMEOUT", "oops")
    with pytest.raises(MODULE.TelemetryError):
        MODULE.parse_args([])


def test_parse_args_uses_env_timeout(monkeypatch):
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_TIMEOUT", "42")
    args = MODULE.parse_args([])
    assert args.timeout == 42.0


def test_parse_args_prefers_cli_over_env(monkeypatch):
    monkeypatch.setenv("SUGARKUBE_TELEMETRY_TIMEOUT", "99")
    args = MODULE.parse_args(["--timeout", "12"])
