...
def _alert_checks(environment, name, kind, pending_time, firing_time):
    ...
    annotations = {
        ...
        "description": (
            "The application-owned essential visitor contract failed; optional "
            "immersive-rendering availability is not part of this alert."
            if kind == "failure"
            else "An enabled producer has no current sanitized result; an intentionally "
            "disabled producer does not alert."
        ),
        "remediation": (
            "Inspect only the bounded failure-stage metric, then qualify recovery before "
            "changing activation."
            if kind == "failure"
            else "Verify the producer schedule and bounded result handoff without executing "
            "an unauthorized probe."
        ),
        "runbook_url": (
            "https://github.com/futuroptimist/sugarkube/blob/main/"
            "docs/danielsmith-visitor-journey.md#alerts"
        ),
    }
    return [
