...
def test_skips_kicad_install_when_history_has_kicad_but_no_changes(
    ...
    board.write_text("")
    subprocess.run(["git", "add", board.name], cwd=tmp_path, check=True, capture_output=True)
    subprocess.run(
        ["git", "commit", "-m", "add board"],
        cwd=tmp_path,
        check=True,
    ...
    docs_file = tmp_path / "CHANGES.md"
    docs_file.write_text("docs-only update\n")
    subprocess.run(["git", "add", docs_file.name], cwd=tmp_path, check=True, capture_output=True)
    subprocess.run(
        ["git", "commit", "-m", "docs"],
        cwd=tmp_path,
        check=True,
    ...
    assert result.returncode == 0, result.stderr
    assert "KiCad not installed; skipping KiBot checks" in result.stderr
