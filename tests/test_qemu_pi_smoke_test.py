...
def test_run_qemu_times_out_without_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    log_path = tmp_path / "serial.log"
    prepared = MODULE.PreparedImage(
        image_path=tmp_path / "image.img",
        kernel=tmp_path / "kernel8.img",
    ...
    class ImmediateTimer:
        ...
        def cancel(self) -> None:  # noqa: D401 - interface compatibility
            """No-op cancel."""

        def join(self, timeout: float | None = None) -> None:  # noqa: ARG002 - parity
            pass
...
def test_run_qemu_requires_success_markers(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    log_path = tmp_path / "serial.log"
    prepared = MODULE.PreparedImage(
        image_path=tmp_path / "image.img",
        kernel=tmp_path / "kernel8.img",
