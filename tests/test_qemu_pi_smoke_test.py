...
def test_decompress_image_copies_to_new_location(tmp_path: Path) -> None:
...
def test_run_invokes_subprocess(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(command, **kwargs):  # noqa: ANN001 - matches subprocess API
...
def test_collect_reports_replaces_existing_directories(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    image = tmp_path / "image.img"
    image.write_bytes(b"data")
    ...
    dest = tmp_path / "artifacts"
    (dest / "first-boot-report").mkdir(parents=True)
    (dest / "first-boot-report" / "old.txt").write_text("old\n")
    (dest / "sugarkube-state").mkdir()
    (dest / "sugarkube-state" / "stale.txt").write_text("stale\n")

    MODULE.collect_reports(image, tmp_path, dest)

    summary = dest / "first-boot-report" / "summary.json"
    assert summary.read_text() == "{}\n"
    assert not (dest / "first-boot-report" / "old.txt").exists()
    assert (dest / "sugarkube-state" / "first-boot.ok").exists()
    assert not (dest / "sugarkube-state" / "stale.txt").exists()
