...
def test_attach_loop_requires_device(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_run_qemu_kills_hung_process(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    log_path = tmp_path / "serial.log"
    prepared = MODULE.PreparedImage(
        image_path=tmp_path / "image.img",
        kernel=tmp_path / "kernel8.img",
    ...
    class HangingProcess:
        ...
        def __iter__(self) -> "HangingProcess":
            return self
        ...
        def terminate(self) -> None:
            # Process ignores termination and continues running.
            pass
...
def test_prepare_image_requires_kernel(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    image = tmp_path / "sugarkube.img"
    image.write_bytes(b"")
    ...
    with pytest.raises(MODULE.SmokeTestError):
        MODULE.prepare_image(image, tmp_path)


def test_prepare_image_requires_cmdline(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    image = tmp_path / "sugarkube.img"
    image.write_bytes(b"")
    ...
    def fake_run(command, **_):
        if command[:2] == ["losetup", "--find"]:
            return subprocess.CompletedProcess(command, 0, stdout="/dev/loop4\n", stderr="")
        return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
...
def test_collect_reports_overwrites_existing_targets(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    image = tmp_path / "image.img"
    image.write_bytes(b"data")
    ...
    def fake_run(command, **_):
        if command[:2] == ["losetup", "--find"]:
            return subprocess.CompletedProcess(command, 0, stdout="/dev/loop5\n", stderr="")
        return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
