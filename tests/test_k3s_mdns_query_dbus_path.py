...
def test_k3s_mdns_query_gdbus_uses_correct_path():
    ...
    assert '"--object-path",' in script_content, "gdbus command not found in k3s_mdns_query.py"
...
def test_k3s_mdns_query_busctl_uses_correct_path():
        ...
        if '"busctl",' in line or "busctl" in line:
            ...
            if "ServiceBrowserNew" in context:
                ...
                if (
                    ...
                    or '"/org/freedesktop/Avahi/Server"' in context
                ):
    ...
    assert (
        found_busctl_correct
    ), (
        "Could not find correct /org/freedesktop/Avahi/Server path in "
        "busctl ServiceBrowserNew command"
    )
