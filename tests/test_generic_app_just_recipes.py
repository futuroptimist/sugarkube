...
TOKENPLACE_PHASE_ONE_METRIC_FAMILIES = [
...
def test_tokenplace_phase_one_metric_families_cover_both_environments():
    environments = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))[
        "applications"
    ]["tokenplace"]["environments"]

    for environment in ("staging", "prod"):
        assert environments[environment]["requiredMetricFamilies"] == (
            TOKENPLACE_PHASE_ONE_METRIC_FAMILIES
        )
...
def test_observability_app_metrics_verify_tokenplace_prod_phase_one_fixture_passes(
    monkeypatch, capsys
):
    cfg = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))["applications"][
        "tokenplace"
    ...
    _verify_base(
        monkeypatch,
        cfg,
        _tokenplace_metric_fixture(cfg, set(TOKENPLACE_PHASE_ONE_METRIC_FAMILIES)),
    )
...
@pytest.mark.parametrize("missing", TOKENPLACE_PHASE_ONE_METRIC_FAMILIES)
def test_observability_app_metrics_verify_tokenplace_prod_requires_each_phase_one_family(
    monkeypatch, missing
):
    cfg = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))["applications"][
        "tokenplace"
    ...
    cfg = {**cfg, "retries": {"attempts": 1, "delaySeconds": 0}}
    present = set(TOKENPLACE_PHASE_ONE_METRIC_FAMILIES) - {missing}
...
def test_observability_app_metrics_verify_tokenplace_prod_rejects_unbounded_label(monkeypatch):
    ...
    _verify_base(
        ...
        _tokenplace_metric_fixture(
            cfg,
            set(TOKENPLACE_PHASE_ONE_METRIC_FAMILIES),
