...
if [ "${{1:-}}" = {str(REPO_ROOT / 'scripts/dspace_release_manifest.py')!r} ]; then
  printf 'release-manifest %s\\n' "${{2:-}}" >> {str(tmp_path / 'commands.log')!r}
fi
...
if [[ "$*" == *"status dspace --namespace dspace -o json" ]]; then
  ...
  if [[ "$*" == *"staging-kubeconfig"* ]]; then
    revision="${{SUGARKUBE_STUB_STAGING_HELM_REVISION:-7}}"
  fi
  printf '{{"name":"dspace","namespace":"dspace","version":%s,"info":{{"status":"deployed","description":"%s"}},"chart":{{"metadata":{{"name":"dspace","version":"%s"}}}}}}\n' "$revision" "$description" "$chart_version"
  exit 0
fi
if [[ "$*" == *"get values"* ]]; then
...
def test_dspace_promote_prod_routes_sparse_named_arguments_through_both_gates(
    ...
    (tmp_path / "commands.log").write_text("", encoding="utf-8")
    result = _run_just(
        [
            "app-promote-prod",
            "app=dspace",
...
@pytest.mark.usefixtures("ensure_just_available")
@pytest.mark.parametrize("staging_record_kind", ["candidate", "coordinate", "revision"])
def test_dspace_prod_staging_gate_failures_stop_before_production_work(
    staging_record_kind: str,
    tmp_path: Path,
    generic_app_stub_env: dict[str, str],
) -> None:
    staging_manifest = tmp_path / "staging-candidate.json"
    ...
    prod_evidence = tmp_path / "prod-evidence.json"
    _write_dspace_candidate(staging_manifest, "staging")
    if staging_record_kind == "candidate":
        staging_evidence.write_text(staging_manifest.read_text(encoding="utf-8"), encoding="utf-8")
    else:
        staged = _run_just(
            [
                "app-deploy",
        ...
        assert staged.returncode == 0, staged.stderr + staged.stdout
        if staging_record_kind == "coordinate":
            record = json.loads(staging_evidence.read_text(encoding="utf-8"))
            record["chartDigest"] = "sha256:" + "9" * 64
            staging_evidence.write_text(json.dumps(record) + "\n", encoding="utf-8")
    ...
    assert not Path(str(prod_evidence.resolve()) + ".reservation").exists()


@pytest.mark.usefixtures("ensure_just_available")
def test_dspace_prod_verifier_failure_preserves_post_mutation_reservation(
    tmp_path: Path, generic_app_stub_env: dict[str, str]
) -> None:
    staging_manifest = tmp_path / "staging-candidate.json"
    staging_evidence = tmp_path / "staging-evidence.json"
...
@pytest.mark.usefixtures("ensure_just_available")
@pytest.mark.parametrize(
    ("recipe", "leading_arguments"),
    [
        ("app-deploy", ["app=dspace", "env=prod"]),
        ("app-redeploy", ["app=dspace", "env=prod"]),
        ("app-promote-prod", ["app=dspace"]),
        ("dspace-oci-deploy", ["env=prod"]),
        ("dspace-oci-redeploy", ["env=prod"]),
        ("dspace-oci-promote-prod", []),
        ("dspace-oci-deploy-prod-subdomain", []),
    ],
)
def test_dspace_production_wrappers_share_staging_evidence_guard(
    recipe: str,
    leading_arguments: list[str],
    tmp_path: Path,
    generic_app_stub_env: dict[str, str],
) -> None:
    manifest = tmp_path / f"{recipe}-candidate.json"
    evidence = tmp_path / f"{recipe}-evidence.json"
    ...
    commands_path = tmp_path / "commands.log"
...
TOKENPLACE_MODERN_METRIC_FAMILIES = [
    "tokenplace_compute_nodes_registered",
    "tokenplace_compute_nodes_healthy",
...
def test_observability_app_metrics_inventory_tokenplace_contract_is_environment_specific():
...
def _tokenplace_metric_fixture(cfg, missing_family=None, extra_labels=None):
    def prom_func(path):
        if path == "/api/v1/targets":
            return {
                "activeTargets": [
        ...
        labels = {
            "__name__": metric,
            "app": "tokenplace",
            "environment": cfg["targetLabels"]["environment"],
            "version": "main-deadbee",
            "revision": "main-deadbee",
            **(extra_labels or {}),
        }
...
def test_observability_app_metrics_production_maintenance_fixture_passes(monkeypatch, capsys):
    cfg = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))["applications"][
        "tokenplace"
    ]["environments"]["prod"]
    cfg["retries"] = {"attempts": 1, "delaySeconds": 0}
    _verify_base(monkeypatch, cfg, _tokenplace_metric_fixture(cfg))

    app_metrics.verify("tokenplace", "prod")

    captured = capsys.readouterr()
    assert "credential" not in (captured.out + captured.err).lower()


@pytest.mark.parametrize("missing_family", TOKENPLACE_MAINTENANCE_METRIC_FAMILIES)
def test_observability_app_metrics_production_requires_each_maintenance_family(
    monkeypatch, missing_family
):
    cfg = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))["applications"][
        "tokenplace"
    ]["environments"]["prod"]
    ...
    _verify_base(
        monkeypatch,
        cfg,
        _tokenplace_metric_fixture(cfg, missing_family=missing_family),
    )

    with pytest.raises(app_metrics.Error, match=f"required metric family missing: {missing_family}"):
        app_metrics.verify("tokenplace", "prod")


def test_observability_app_metrics_production_rejects_unbounded_application_label(monkeypatch):
    cfg = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))["applications"][
        "tokenplace"
    ]["environments"]["prod"]
    _verify_base(
        monkeypatch,
        cfg,
        _tokenplace_metric_fixture(cfg, extra_labels={"request_id": "not-allowed"}),
    )
...
def test_observability_app_metrics_staging_rejects_maintenance_only_fixture(monkeypatch):
    cfg = json.loads(APP_METRICS_CONFIG.read_text(encoding="utf-8"))["applications"][
        "tokenplace"
    ]["environments"]["staging"]
    cfg["retries"] = {"attempts": 1, "delaySeconds": 0}
    missing_modern = set(TOKENPLACE_MODERN_METRIC_FAMILIES).difference(
        TOKENPLACE_MAINTENANCE_METRIC_FAMILIES
    )

    def maintenance_only(path):
        metric = app_metrics.urllib.parse.unquote(path.rsplit("query=", 1)[-1]).split(
            "{", 1
        )[0]
        family = app_metrics.metric_family_from_series(metric)
        return _tokenplace_metric_fixture(
            cfg, missing_family=family if family in missing_modern else None
        )(path)

    _verify_base(monkeypatch, cfg, maintenance_only)
...
def test_gitshelves_example_config_resolves_all_environments(env: str) -> None:
    result = subprocess.run(
        ["python3", "scripts/app_config.py", "json", "--app", "gitshelves", "--env", env],
        cwd=REPO_ROOT, capture_output=True, text=True, check=False,
    )
    assert result.returncode == 0, result.stderr
    assert '"SUGARKUBE_CHART": "oci://ghcr.io/futuroptimist/charts/gitshelves"' in result.stdout
    assert f'"SUGARKUBE_ENV": "{env}"' in result.stdout


def test_gitshelves_values_and_pins_are_safe_and_exact() -> None:
    paths = [REPO_ROOT / f"docs/examples/gitshelves.values.{env}.yaml" for env in ("dev", "staging", "prod")]
    dev, staging, prod = [path.read_text(encoding="utf-8") for path in paths]
    combined = "\n".join((dev, staging, prod)).lower()
    assert "repository: ghcr.io/futuroptimist/gitshelves" in dev
    assert "tag: main-REPLACE_SHORTSHA" in dev
    ...
    assert "letsencrypt-production" in staging and "letsencrypt-production" in prod
    assert (REPO_ROOT / "docs/apps/gitshelves.version").read_text().splitlines()[-1] == "0.1.0"
    assert not [line for line in (REPO_ROOT / "docs/apps/gitshelves.prod.tag").read_text().splitlines() if line.strip() and not line.lstrip().startswith("#")]
    for forbidden in ("secretkeyref", "kind: secret", "persistence:", "persistentvolumeclaim", "database", "queue:"):
        assert forbidden not in combined


@pytest.mark.usefixtures("ensure_just_available")
def test_app_verify_print_only_gitshelves_paths(generic_app_stub_env: dict[str, str]) -> None:
    result = _run_just(["app-verify", "app=gitshelves", "env=staging", "print_only=1"], generic_app_stub_env)
    assert result.returncode == 0, result.stderr + result.stdout
    assert result.stdout.splitlines() == [
        "curl -fsS https://example.test/",
        "curl -fsS https://example.test/healthz",
...
@pytest.mark.parametrize("tag", ["latest", "main", "main-latest", "staging", "v1.2.3"])
def test_gitshelves_generic_deploy_rejects_moving_tags(tag: str, generic_app_stub_env: dict[str, str]) -> None:
