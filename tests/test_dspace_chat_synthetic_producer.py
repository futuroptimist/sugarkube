...
def test_retained_asset_accepts_historical_runtime_directory_contract(tmp_path: Path) -> None:
    retained = tmp_path / "retained"
    current_manifest = installer.render(retained)
    assert installer.validate_current_candidate(retained) == current_manifest
    service = retained / "etc/systemd/system/dspace-chat-synthetic.service"
    service.write_text(service.read_text().replace("RuntimeDirectoryPreserve=yes\n", ""))
    manifest = json.loads((retained / "manifest.json").read_text())
    relative = "etc/systemd/system/dspace-chat-synthetic.service"
    manifest[relative] = installer.sha(service)
    (retained / "manifest.json").write_text(json.dumps(manifest, sort_keys=True, indent=2) + "\n")

    assert installer.validate_retained_asset(retained) == manifest
    assert not installer.classification_runtime_directory_preserved(retained)
    with pytest.raises(ValueError, match="classification runtime directory is not preserved"):
...
def test_same_revision_manifest_migration_preserves_runner_and_rolls_back_exactly(
    ...
    old_service_relative = "etc/systemd/system/dspace-chat-synthetic.service"
    old_manifest[old_service_relative] = installer.sha(old_service)
    ...
    old_retained.parent.mkdir(parents=True, exist_ok=True)
