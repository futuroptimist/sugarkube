...
def test_check_service_via_dbus_uses_correct_path():
    ...
    assert (
        "check_service_via_dbus" in script_content
    ...
    assert not (
        '"--object-path",\n                "/",' in script_content
        and '"org.freedesktop.Avahi.Server.ResolveService"' in script_content
...
def test_mdns_publish_static_python_section_correct_path():
    ...
    assert (
        "def run_command" in script_content or "import subprocess" in script_content
