  ...
  case "$1" in
    hosts)
      if [ "$2" = "sugarkube0.local" ]; then
        echo "192.168.50.10 sugarkube0.local"
        exit 0
      fi
      ;;
    ahostsv4)
      if [ "$2" = "sugarkube0.local" ]; then
        echo "192.168.50.10 STREAM sugarkube0.local"
        exit 0
      fi
      ;;
  esac
...
cat <<'OUT'
=;eth0;IPv4;k3s API sugar/dev [server] on sugarkube0;_k3s-sugar-dev._tcp;local;sugarkube0.local;;6443;txt=k3s=1;txt=cluster=sugar;txt=env=dev;txt=role=server;txt=phase=ready
OUT
exit 0
SH
...
if ! grep -q 'SERVER_IP=192.168.50.10' "${INSTALL_LOG_COPY}"; then
  echo "SERVER_IP did not capture NSS address" >&2
  exit 1
fi
