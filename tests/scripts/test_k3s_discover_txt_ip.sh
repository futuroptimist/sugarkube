...
#!/usr/bin/env bash
set -eo pipefail
if [ "$#" -ge 4 ] && [ "$1" = "-j" ] && [ "$2" = "-4" ] && [ "$3" = "addr" ] && [ "$4" = "show" ]; then
  cat <<'OUT'
[
  {
    "ifname": "eth0",
...
if [ "$#" -ge 4 ] && [ "$1" = "-j" ] && [ "$2" = "-6" ] && [ "$3" = "addr" ] && [ "$4" = "show" ]; then
  cat <<'OUT'
[
  {
    "ifname": "eth0",
...
if [ "$#" -ge 4 ] && [ "$1" = "-6" ] && [ "$2" = "-o" ] && [ "$3" = "addr" ] && [ "$4" = "show" ]; then
  cat <<'OUT'
...
printf '%s\n' "$*" >> "${AVAHI_LOG}"
cat <<'OUT'
=;eth0;IPv4;k3s API sugar/dev [server] on sugarkube0;_k3s-sugar-dev._tcp;local;sugarkube0.local;;6443;txt=k3s=1;txt=cluster=sugar;txt=env=dev;txt=role=server;txt=phase=ready;txt=ip4=192.168.50.10;txt=ip6=2001:db8::20
OUT
exit 0
SH
...
if ! grep -q 'SERVER_IP=192.168.50.10' "${INSTALL_LOG_COPY}"; then
