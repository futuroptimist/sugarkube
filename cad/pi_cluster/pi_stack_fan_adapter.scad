...
fan_side_x = len(stack_mount_positions) > 0 ? max([for (p = stack_mount_positions) p[0]]) : 0;
...
fan_side_span = len(fan_side_positions) > 0
    ? max([for (p = fan_side_positions) p[1]]) - min([for (p = fan_side_positions) p[1]])
    : 0;
body_span_y = fan_side_span + 20;
body_height = stack_height;
center_y = len(fan_side_positions) > 0
    ? (max([for (p = fan_side_positions) p[1]]) + min([for (p = fan_side_positions) p[1]])) / 2
    : 0;
