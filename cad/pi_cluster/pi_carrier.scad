...
stack_pocket_depth_input = is_undef(stack_pocket_depth)
...
stack_mount_inset = is_undef(stack_mount_inset)
    ? max(corner_radius + stack_pocket_d / 2 + 2, 12)
    : stack_mount_inset;
...
assert(
    ...
    "stack_mount_inset must keep mounts inside the plate bounds"
);
