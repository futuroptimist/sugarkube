...
- **Remotely-managed** tunnels (token-only, created in the Cloudflare dashboard). This is what
  Sugarkube uses.
- **Locally-managed** tunnels (created with `cloudflared login` + `cert.pem`). This guide does **not**
  cover that flow.

If you follow the Cloudflare dashboard flow below, you are already in the remotely-managed model.
...
- On `sugarkube0`, export `CF_TUNNEL_TOKEN` (and optionally `CF_TUNNEL_NAME`), then run
  `just cf-tunnel-install env=<dev|staging> token="$CF_TUNNEL_TOKEN"`.
- In the tunnel UI, add a Public hostname routing `staging.democratized.space` to
...
5. Name the tunnel (for example, `dspace-staging-v3`; pick any name you like).
6. Click **Save tunnel**. The dashboard shows an **Install and run a connector** panel with
   OS-specific commands (Windows/Mac/Debian/Docker, etc.). **Do not run these commands on your
   workstation or Pi**—Sugarkube will run `cloudflared` inside the cluster for you. Your job here is
   only to copy the token.

   The panel shows commands like:
   ...
   All of these commands embed the **same** tunnel token. Copy only the `<TUNNEL_TOKEN>` portion—the
   long string starting with `eyJ...`. You can grab it from **any** command on this page; they all
   use the same token.
...
This corresponds to the **Install and run a connector** step in Cloudflare. Sugarkube deploys
`cloudflared` as a pod inside the cluster using the tunnel token you copied. No `cert.pem` or
`credentials.json` is used in this remotely-managed flow.
...
- **Sugarkube `env`** (for example, `dev`, `staging`): Sugarkube’s environment name (labels, defaults
  such as `sugarkube-<env>`); it does **not** decide which Cloudflare tunnel you join.
- **CF_TUNNEL_NAME**: optional override so the in-cluster name matches the Cloudflare dashboard name.

Sugarkube joins the tunnel specified by the token, optionally aligning the in-cluster name via
`CF_TUNNEL_NAME`. It is safe to run a staging tunnel on a cluster where `env=dev` so long as the
token (and optionally `CF_TUNNEL_NAME`) come from that staging tunnel.

Example name combinations:

| Sugarkube `env` | CF_TUNNEL_NAME        | Resulting tunnel joined |
| --------------- | --------------------- | ----------------------- |
| `dev`           | (unset)               | Token’s tunnel name     |
| `dev`           | `dspace-staging-v3`   | `dspace-staging-v3`     |
| `staging`       | `dspace-staging-v3`   | `dspace-staging-v3`     |

### Deploy the Helm chart via Sugarkube

Run these commands on the Pi (for example, `sugarkube0`):

1. Point `kubectl` at the cluster and switch to the Sugarkube checkout:
...
2. Export the tunnel token (copy-pasted from the dashboard) and optional naming overrides. The token
   is the `eyJ...` value from any `cloudflared ... <TUNNEL_TOKEN>` command in the **Install and run a
   connector** panel:

   ```bash
   export CF_TUNNEL_TOKEN="eyJ...copy-pasted-from-dashboard..."
   export CF_TUNNEL_NAME="dspace-staging-v3"   # Optional: align with the dashboard name
   export CF_TUNNEL_ID="<dashboard-tunnel-id>" # Optional: helpful for alignment, not required
   ```

   `env=dev` / `env=staging` refers to the Sugarkube environment, not a Cloudflare concept.
...
3. Install or update the chart and Secret on the cluster (namespace is created if needed):
   ...
   Passing `token=` keeps the intent obvious; omitting it falls back to `CF_TUNNEL_TOKEN` in the
   environment. The recipe strips common prefixes (`token=<jwt>`, `TUNNEL_TOKEN=<jwt>`, or a full
   `cloudflared ... --token <jwt>` command) and mounts the Secret directly as `TUNNEL_TOKEN`. The
   chart runs `cloudflared tunnel --no-autoupdate --loglevel debug --metrics 0.0.0.0:2000 run` with
   **no** origin certificate references.
...
### Worked example: `dspace-staging-v3` tunnel on the `dev` Sugarkube env

Full sequence on `sugarkube0`:
...
# Token from the Cloudflare dashboard (any command in the Install and run a connector panel).
...
`dspace-staging-v3` tunnel and routes `staging.democratized.space` because both the token and
...
`cert.pem`/`TUNNEL_ORIGIN_CERT`, not the token-only remotely-managed mode. Common causes:

- The tunnel in Cloudflare was created with the older locally-managed flow (config + `cert.pem`).
- The Deployment references a config file or secret from a locally-managed tunnel.
- `CF_TUNNEL_TOKEN` (or the `tunnel-token` Secret) does not actually contain the token for this
  tunnel—for example, it is a different API token or a token from another tunnel.
