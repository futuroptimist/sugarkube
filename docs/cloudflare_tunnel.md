...
This is the “Install and run a connector” step the Cloudflare UI prompts for. It **must** run on a
node in the k3s cluster (for example, `sugarkube0`), not on your workstation. Use Sugarkube to
install and manage the connector.
...
1. Export the tunnel token (add optional name and ID overrides to keep dashboard names aligned):
   ```bash
   export CF_TUNNEL_TOKEN="<tunnel-token>"              # From the dashboard in Step 1
...
cluster so requests can flow through the tunnel to Traefik.
...
automatically that points `staging.democratized.space` to your tunnel’s `*.cfargotunnel.com` address.
If you see a DNS record for `staging.democratized.space` pointing at `<UUID>.cfargotunnel.com`, you’re
done.
...
- A named Cloudflare Tunnel exists in the dashboard (for example, `dspace-staging-k3s`).
- `cloudflared` is installed and running in the k3s cluster via `just cf-tunnel-install` using that
  tunnel token.
- The tunnel routes `staging.democratized.space` to the Traefik ClusterIP service inside the
  cluster.
- Cloudflare DNS has a proxied CNAME pointing `staging.democratized.space` at the tunnel’s
