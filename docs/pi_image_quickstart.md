...
the `99-sugarkube-ssd-clone.rules` udev rule whenever a USB or NVMe disk is attached, or it
can be started manually. It is intentionally not tied to `multi-user.target`, so normal boot
continues even when no SSD is present. Inspect the journal to monitor progress:
