...
- Runs a Gateway API CRD ownership preflight. Missing CRDs are fine; conflicts stop the install with
  guidance to run `just traefik-crd-doctor` for remediation.
- Installs or upgrades the Traefik Helm release (`traefik/traefik`) in `kube-system` with
  `helm upgrade --install ... --wait --timeout=5m` and enables the Traefik Gateway API controller.
...
Traefik's Gateway API CRDs must be managed by Helm. The recipe checks that the CRDs are owned by a
Traefik release in `kube-system` and fails fast with remediation commands if other owners are found.
Missing CRDs are acceptable and will be created by the main chart. If `just traefik-install`
reports ownership problems, run the doctor before retrying the install.
