...
## Fix kubectl access on each Pi

Fresh k3s installs keep `/etc/rancher/k3s/k3s.yaml` root-owned. When `kubectl` runs as `pi`, it
falls back to `localhost:8080` and fails because no API server is listening there.

On each node, run the one-liner below once after k3s is installed:

```bash
just kubeconfig
```

This creates `~/.kube/config`, sets the correct permissions, and persists
`KUBECONFIG=$HOME/.kube/config` in `~/.bashrc`. Open a new shell (or `source ~/.bashrc`) before
retrying `kubectl`.
...
> This section expands step 3 of the golden path above (validating Traefik).
...
ingress controller. Traefik ships with k3s by default via addon HelmCharts, so you typically only
need to verify that it is running.
...
If Traefik pods or services are missing, confirm the k3s addon objects exist in `kube-system`
(`traefik` and `traefik-crd` HelmChart objects, plus `helm-install-traefik-*` pods). Missing addon
objects usually indicate k3s was installed with Traefik disabled; re-enable the addon or proceed
with the advanced override below.

**Advanced override (manual Traefik management)**

If you intentionally manage Traefik yourself (for example, disabled the k3s addon), you can use the
`just traefik-install` escape hatch to install the official Helm chart. This is not the default
path for sugarkube; verify the addon first.
...
are owned by a Traefik release in `kube-system`. Missing CRDs are acceptable—the main
`traefik/traefik` chart will create them—and already healthy, Traefik-owned CRDs are also fine. If
you run `just traefik-install` and see ownership problems, run the doctor before retrying the
manual override.
...
- Missing CRDs (safe for Traefik to create):
  ...
  Next step: run 'just traefik-status' to verify Traefik is running (k3s installs it by default).
  ```
...
remain, then re-run `just traefik-status` to verify Traefik. If you are managing Traefik manually,
re-run `just traefik-install` as the override.
...
Before relying on Traefik in the ha3 topology, run this helper so the three control-plane nodes can
...
and Traefik is ready before deploying apps like dspace. For the underlying `kubectl` and `helm`
