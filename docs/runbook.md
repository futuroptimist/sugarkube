...
For scripted assurance, combine the D-Bus helper with wire-proofing so the
double-negative absence gate fully clears before you retry:

```bash
SUGARKUBE_MDNS_DBUS=1 \
SUGARKUBE_MDNS_WIRE_PROOF=1 \
SUGARKUBE_DEBUG_MDNS=1 \
scripts/mdns_selfcheck.sh
```

The self-check waits for a confirmed 6443 listener before reporting success and
logs `ms_elapsed=<n>` so you can spot outliers (see [LOGGING.md](LOGGING.md)). On
quiet wired LANs the end-to-end time typically stays under 750 ms; multi-second
values hint at multicast contention or RFC 6762 suppression backoffs.
...
- The default Flannel CNI (installed by k3s) shows `cni0` bridges on every node
  and `kube-system` pods such as `flannel-*` are `Ready`, ensuring multicast
  used by mDNS stays within RFC 6762 guidelines.
...
`just status` internally waits on the `_https._tcp:6443` readiness gate before
declaring success so workloads do not advance until the Kubernetes API is
reachable.
