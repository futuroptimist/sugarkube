...
any apps. Traefik ships with k3s by default; in sugarkube we rely on that.
...
If Traefik pods or services are missing, inspect the k3s addon objects and HelmChart
installer pods in `kube-system` for clues.
...
playbooks (`token.place`, `dspace`, etc.) as usual. k3s installs Traefik by default, so verify
ingress health before rolling out HTTP applications.

Follow the [Traefik ingress verification][traefik-ingress] steps before rolling out HTTP
applications.
