...
- This document does not itself prove runtime deployment. Source manifests may install or configure runtime components, and live claims require separate verification evidence.
...
| --- | --- | --- | --- | --- |
| Sugarkube cluster and Pi image | Pi image compose includes node exporter, cAdvisor, Grafana Agent, and Netdata containers, with Grafana Agent aggregating node and cAdvisor metrics to `:12345`; docs mention scraping `:9100`, `:12345`, and Netdata `:19999` ([compose](../scripts/cloud-init/docker-compose.yml), [Flow config](../scripts/cloud-init/observability/grafana-agent.river), [telemetry docs](./pi_image_telemetry.md), [projects compose](./projects-compose.md)). The Pi telemetry publisher is opt-in and anonymizes stable identifiers before posting verifier summaries ([telemetry docs](./pi_image_telemetry.md)). Separately, a live, guarded non-Flux staging `kube-prometheus-stack` + blackbox-exporter lifecycle is deployed and scripted-verified, with the shared Grafana dashboard provisioned as code ([operations runbook](./observability-operations.md), [blackbox runbook](./observability-blackbox.md)). | Flux-style `platform/observability` manifests define kube-prometheus-stack, Loki, and Promtail with Prometheus retention/storage values and Alertmanager `null-receiver`, but this audit did not verify those *Flux* manifests specifically are deployed on a live cluster ([HelmRelease](../platform/observability/kube-prometheus-stack.yaml), [values](../platform/observability/kube-prometheus-stack-values.yaml), [Loki](../platform/observability/loki.yaml)) — they are distinct from, and must not be combined with, the live staging Helm lifecycle noted above. | Retain the guarded non-Flux lifecycle, ServiceMonitor/Probe discovery, and existing shared dashboard while adding alert routes and app metrics in later slices. Any future migration to Flux ownership must explicitly replace that lifecycle rather than concurrently manage the same releases. | Installing runtime components in this PR; claiming production Grafana/Prometheus operations experience before release evidence exists. |
| DSPACE | Public app repo has `/healthz` and `/livez` health endpoints, Docker healthcheck, Helm chart probes, image/chart workflows, and a local Docker Compose Prometheus/Grafana monitoring scaffold. The dedicated `infra/metrics.mjs` listener is enabled separately and currently serves `/metrics` without bearer-token enforcement at the app level; the documented DSPACE `METRICS_TOKEN` and ServiceMonitor bearer-token intent must not be treated as verified protection for that dedicated metrics port until DSPACE implements and tests it. The metrics port must be protected by Kubernetes network policy, cluster-only Service exposure, ingress rules, or an application-side auth change before use outside trusted local monitoring ([DSPACE Dockerfile](https://github.com/democratizedspace/dspace/blob/main/Dockerfile), [metrics server](https://github.com/democratizedspace/dspace/blob/main/infra/metrics.mjs), [monitoring README](https://github.com/democratizedspace/dspace/tree/main/infra/monitoring), [chart](https://github.com/democratizedspace/dspace/tree/main/charts/dspace)). Sugarkube runbooks verify `/config.json`, `/healthz`, and `/livez` ([runbook](./apps/dspace.md), [app contract](./app_deployment_contract.md)). In staging, DSPACE's authenticated `ServiceMonitor` (chart `3.1.2`) is deployed with an Alertmanager-independent bearer-token Secret and is being scraped successfully by Prometheus; operators have manually observed two healthy targets ([operations runbook](./observability-operations.md)). The five release-integrity rules, route, dashboard section, bounded consumer, and pinned five-minute producer are deployed, and all five alerts are healthy; the drill proved firing, acknowledgement, resolution, and exact cleanup for three alerts ([release-integrity runbook](./observability-dspace-release-integrity.md)). | Metrics endpoint behavior at the application layer was verified from source, not by hitting a deployed DSPACE pod directly. Existing monitoring scaffold is local/app-owned and not proof of Sugarkube production Prometheus. | A separate future production-observability rollout; the staging proof is not production evidence. | Recording prompts, answers, player save data, inventory, user identity, or unbounded errors in metrics/logs. |
...
| Cloudflare ingress/tunnels | `platform/cloudflared` values enable Cloudflared metrics and route `apps.sugarkube.dev` to Traefik, while serviceMonitor is disabled ([config](../platform/cloudflared/configmap.yaml)). Cloudflare runbooks document metrics/ready port checks ([tunnel runbook](./cloudflare_tunnel.md)). The repository-defined blackbox matrix contains 21 probes across five supported apps: DSPACE, token.place, danielsmith.io, jobbot3000, and GitShelves. Previously verified live evidence covers the 16 non-GitShelves probes; the five GitShelves probes are not live evidence until its workload, Cloudflare route, and Probes are separately applied ([blackbox runbook](./observability-blackbox.md)). The blackbox exporter runs inside the cluster and reaches those public URLs through cluster DNS and egress; these probes do not provide an independent external-client vantage point. | This audit did not independently verify live Cloudflare tunnel/DNS/Access policy behavior beyond what those in-cluster probes exercise. In particular, their success does not prove routes are reachable from networks outside the cluster. | Cloudflare tunnel metrics scraping when enabled, an external-client probe for independent reachability coverage, and runbooks for separating DNS/tunnel failures from app releases. | Managing Cloudflare/DNS from app Helm charts; public Prometheus; public Grafana until an explicit later phase with Zero Trust. |
| GitShelves | Five staging probes cover `root`, `healthz`, `livez`, `baseplate`, and `module` after the workload and external route exist ([blackbox runbook](./observability-blackbox.md), [app runbook](./apps/gitshelves.md)). | Static browser-only application with no `/metrics` endpoint; no ServiceMonitor is required. | Revisit only if app-owned metrics are introduced. | Browser-local data must never become monitoring labels or payloads. |
| jobbot3000 | Covered by the same staging blackbox matrix as the other onboarded apps: `root`, `healthz`, `livez`, `tracker`, and `manifest` probes are implemented and scripted-verified ([blackbox runbook](./observability-blackbox.md), [app runbook](./apps/jobbot3000.md)). | This audit did not verify an app-owned `/metrics` endpoint or `ServiceMonitor` for jobbot3000. | An app-specific metrics/dashboard/alert story, matching the depth of §7-§9 below, once jobbot3000 exposes application metrics. | Same privacy/cardinality constraints as the other three apps once app metrics exist. |
...
graph TD
  ...
  subgraph K3s[Sugarkube k3s cluster]
    ...
    subgraph Apps[application namespaces]
      ...
      J[jobbot3000 /healthz /livez and optional content endpoints]
  ...
  T --> SM
  DS --> SM
  J -. blackbox only until app metrics exist .-> BBTargets
  KSM --> Prom
  Node --> Prom
  Cad --> Prom
  API --> Prom
...
- App-owned ServiceMonitors for `dspace`, `tokenplace`, and `danielsmith` should use a `namespaceSelector` that names the corresponding app namespace, or an equivalent platform-owned selector that is explicit about those namespaces. jobbot3000 remains blackbox-only until it exposes application metrics; add ServiceMonitor discovery only when such metrics exist.
...
- Prometheus requires persistent storage in production. Initial production Grafana persistence is disabled, so UI-created state is ephemeral.
...
DSPACE has `/healthz` and `/livez` support in Docker/Helm/runbooks, release workflows for GHCR image and chart publication, and an app-local `/metrics` implementation and monitoring scaffold in the public repo. Sugarkube currently verifies `/config.json`, `/healthz`, and `/livez` through app runbooks and the shared app contract. In staging, DSPACE's authenticated `ServiceMonitor` is deployed and is being scraped successfully by Prometheus, with two healthy targets manually observed by operators (see [`docs/observability-operations.md`](./observability-operations.md)). The bounded `/chat` producer and consumer, five canonical rules, PagerDuty route, and dashboard section are deployed in staging, and all five alerts are healthy; the owner-scoped drill proved live firing, acknowledgement, and resolution for three alerts, followed by exact cleanup (see the [`release-integrity runbook`](./observability-dspace-release-integrity.md)). Production observability remains a separate future rollout and is not claimed by that proof.
...
- Public availability: the active staging manifest, `clusters/staging/observability/probes/public-apps.yaml`, currently supplies the four in-cluster blackbox probes for `https://staging.democratized.space/`, `/config.json`, `/healthz`, and `/livez`. Production probes are future work and require an approved production lifecycle; the staging-only non-Flux lifecycle does not own them.
...
- Alert set: public/API down, sustained zero healthy production compute nodes regardless of current demand, queue age/depth high, stale leases not evicted, upstream dependency unavailable, crash loop, and scrape down. Missing or unhealthy telemetry must remain a separate scrape-target alert rather than being coerced into zero capacity.
...
The staging dashboard described in this section is implemented as code today: `Sugarkube Staging
Observability` (UID `sugarkube-staging-observability`) is Helm-provisioned and re-created on every
apply, and has been proven to survive Grafana pod replacement without manual UI changes (see
[`docs/observability-operations.md`](./observability-operations.md)). The remaining rows below
describe the target set of dashboards, not all of which exist as separate Grafana dashboards yet —
DSPACE, blackbox, and cluster signals currently live together in the one staging dashboard.
...
This table is the platform SLI/alert-signal reference. For delivery architecture, PagerDuty/
Healthchecks.io routing policy, the current alert-name inventory, and the rollout/drill sequence, see
the canonical [`docs/observability-alerting.md`](./observability-alerting.md).
...
| `TokenplaceNoHealthyComputeNodes` | critical in production | explicit healthy-node gauge with healthy scrape and instrumentation | healthy nodes 0, independent of demand | 5m | compute nodes offline, relay registration broken | `docs/k3s-tokenplace-prod.md` | acknowledge; restart/register compute node; confirm resolution |
| `TokenplaceMetricsTargetDown` | critical in production | expected relay workload and Prometheus target | target down or missing while relay runs | 10m | ServiceMonitor discovery, authentication, network | `docs/k3s-tokenplace-prod.md` | restore telemetry; do not infer zero capacity |
...
3. **Blackbox monitoring**: *repository-defined for staging.* The matrix declares 21 public probes across five supported apps, per [`docs/observability-blackbox.md`](./observability-blackbox.md). Previously verified live evidence covers 16 probes. The five GitShelves probes are not claimed live and remain excluded from the WAN dependency-loss drill until its workload, Cloudflare route, and Probes are separately applied.
