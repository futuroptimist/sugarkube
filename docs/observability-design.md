...
- Prefer `ServiceMonitor`/`PodMonitor` labels selected by kube-prometheus-stack.
...
- `/metrics` is enabled in staging and prod behind cluster-only access or a documented token strategy; Prometheus can scrape it without exposing it publicly.
...
| `DanielsmithStaticProbeFailed` | warning/critical | blackbox path probes | `/` or `/resume.pdf` fails | 5m | nginx/static asset/ingress issue | `docs/apps/danielsmith.md` | rollback image/tag; fix ingress if shared |
