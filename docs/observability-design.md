...
| jobbot3000 | Covered by the same staging blackbox matrix as the other three apps: `root`, `healthz`, `livez`, `tracker`, and `manifest` probes are implemented and scripted-verified ([blackbox runbook](./observability-blackbox.md), [app runbook](./apps/jobbot3000.md)). | This audit did not verify an app-owned `/metrics` endpoint or `ServiceMonitor` for jobbot3000. | An app-specific metrics/dashboard/alert story, matching the depth of §7-§9 below, once jobbot3000 exposes application metrics. | Same privacy/cardinality constraints as the other three apps once app metrics exist. |
