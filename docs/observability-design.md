...
2. **Cluster monitoring foundation**: *implemented in staging; production repository lifecycle prepared.* kube-prometheus-stack is deployed and scripted-verified in staging, with resource requests, retention, PV, and Grafana provisioned as documented in [`docs/observability-operations.md`](./observability-operations.md). Production deployment proof and a live-metrics-backed dashboard remain follow-up work; initial production Grafana UI state is intentionally ephemeral.
