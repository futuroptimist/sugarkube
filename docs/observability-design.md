...
- Alert set: public/API down, no healthy compute nodes regardless of current demand, queue age/depth high, stale leases not evicted, upstream dependency unavailable, crash loop, and scrape down.
...
| `TokenplaceNoHealthyComputeNodes` | critical | lease-healthy nodes with healthy scrape and instrumentation | explicitly reported healthy nodes 0 | 5m | compute nodes offline, relay registration broken | `docs/apps/tokenplace.md` | restart/register compute node; rollback relay if release-related |
