...
- Prometheus, Grafana, Alertmanager, blackbox exporter, kube-state-metrics, node/container metrics, and shared dashboard deployment. The first source-level blackbox slice is documented in [`docs/observability-blackbox.md`](./observability-blackbox.md); it is configuration evidence, not live deployment evidence.
...
- ServiceMonitors and Probes should keep the current kube-prometheus-stack release-label convention, such as `release: kube-prometheus-stack`, unless a future runtime PR intentionally migrates Prometheus selector values.
...
- Public availability: blackbox probes for `https://staging.democratized.space/`, `/config.json`, `/healthz`, `/livez`, and production equivalents. Labels use `app=dspace` and bounded route values, not URL labels.
...
- Blackbox probes for `/`, `/livez`, `/healthz`, and the documented `/runtime/github-metrics.json` metadata cache in staging and prod. Mandatory paging/release-blocking blackbox coverage for root `/resume.pdf` starts only after danielsmith.io serves or redirects that path and the runbook/app contract verifies it.
