...
- Alert set: public/API down, no healthy compute nodes regardless of demand, queue age/depth high, stale leases not evicted, upstream dependency unavailable, crash loop, and scrape down.
...
| `TokenplaceNoHealthyComputeNodes` | critical in production | healthy nodes, scrape health, and instrumentation health | explicit healthy nodes 0 with healthy telemetry; no demand gate | 5m | compute nodes offline, relay registration broken | `docs/observability-tokenplace-production.md` | restart/register compute node; rollback relay if release-related |
