...
3. **Blackbox monitoring**: *repository-defined for staging and production.* Each environment declares 21 public probes across five supported apps, per [`docs/observability-blackbox.md`](./observability-blackbox.md). Staging has separate live evidence; production remains repository-ready only until an approved rollout and verification produces evidence.
