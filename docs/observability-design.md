...
DSPACE has `/healthz` and `/livez` support in Docker/Helm/runbooks, release workflows for GHCR image
and chart publication, and an app-local `/metrics` implementation and monitoring scaffold. In
staging, its authenticated `ServiceMonitor` is scraped successfully. The release-integrity dashboard,
five-alert route, and pinned five-minute `/chat` synthetic producer are deployed and healthy; the
owner-scoped three-alert delivery drill completed with acknowledgement, resolution, and exact
cleanup. See the [sanitized verification record](./observability-dspace-release-integrity.md#completed-staging-verification).
No production observability deployment or live test is claimed.
...
6. **Alerts**: the staging synthetic and five-alert DSPACE Alertmanager → PagerDuty routes are
   deployed and delivery-tested; enable additional real alerts only when actionable and linked to
   tested runbooks. See
   [`docs/observability-alerting.md`](./observability-alerting.md) for the rollout sequence.
7. **Staging failure drills**: the synthetic route and the owner-scoped three-alert DSPACE
   fire/acknowledgement/resolution path are proven. Per-node Healthchecks.io installation and
   power-off drills are next.
