...
6. **Alerts**: synthetic Alertmanager-to-PagerDuty fire, acknowledge, and resolve delivery has been proven. Repository-owned, secret-safe per-node Healthchecks.io systemd heartbeats are ready for the post-merge staging rollout documented in [`docs/observability-operations.md`](./observability-operations.md). The Alertmanager watchdog, `KubeNodeNotReady` route, and application alerts remain deferred.
