...
reduce flaky downloads it pins the official Raspberry Pi and Debian mirrors,
 ...
 mirror, and bypasses proxies for `archive.raspberrypi.com`. Override the
 ...
 `APT_RETRIES` and `APT_TIMEOUT` to tune the retry count and per-request
 timeout. Use `BUILD_TIMEOUT` (default: `4h`) to adjust the maximum build
 duration. Customize the cloud-init configuration with
