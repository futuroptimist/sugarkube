...
are installed before running it; `stdbuf` and `timeout` come from GNU coreutils.  ← feature branch
...
// installed before running it; `stdbuf` and `timeout` come from GNU coreutils.  ← main
The script checks that both the temporary and output directories have at least 10 GB free
