...
and telemetry-loss paging. The verifier selects this compatibility contract only
when the live relay reports the approved `sha-f5c6d6b` image tag; every other
production image must satisfy the full 13-family contract. Staging remains on
the modern Qwen relay and retains that full contract.
