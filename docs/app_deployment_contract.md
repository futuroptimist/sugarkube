...
The danielsmith.io staging and production overlays also enable the `githubMetricsCache` sidecar for public POI star counts. The browser reads the generated cache at `/runtime/github-metrics.json`; keep this as a documented manual JSON verification path unless the generic app verifier grows safe, environment-specific JSON assertions. The current public-stars flow is unauthenticated: do not add a GitHub token, Kubernetes Secret, `envFrom` Secret, or credential to app configs or values.
