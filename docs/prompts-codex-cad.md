...
- Use [`scripts/openscad_render.sh`](../scripts/openscad_render.sh) to export binary STL meshes
  into the git-ignored [`stl/`](../stl/) directory (see [`.gitignore`](../.gitignore)). The CI
  workflow [`scad-to-stl.yml`](../.github/workflows/scad-to-stl.yml) regenerates these models
  as artifacts, so never commit `.stl` files. Ensure
...
- Run `pre-commit run --all-files` before committing; it invokes
...
3. Render the model using the script:
