...
- Render each model in all supported `standoff_mode` variants
  (e.g. `heatset`, `printed`, `nut`).
  `STANDOFF_MODE` is case-insensitive, trims surrounding whitespace, and defaults to the model’s
...
3. Render the model via:
   ...
   ```bash
   ./scripts/openscad_render.sh path/to/model.scad  # default (often heatset)
   STANDOFF_MODE=printed ./scripts/openscad_render.sh path/to/model.scad  # ignores case
