...
DSPACE production currently runs application **3.0.1** with chart **3.0.2**. The newly approved
recovery target is immutable chart **3.0.3**, retaining application 3.0.1 and image
`main-1a31a56`. This repository change only onboards that target and its validation contract; it
does not deploy or promote application or chart code. A separately authorized guarded staging
rehearsal and finalized evidence gate are required before any production rollout.
...
3.0.1, select approved chart 3.0.3 at its immutable digest, and retain both replicas; never use
...
the downloaded chart archive's SHA-256. Chart `3.0.3` was published from source revision
`62da11005354e9f9a89c2e58584cdce4c8ec35aa` by upstream PR
`democratizedspace/dspace#4816` and tag `chart-v3.0.3`. Publication workflow run
[31632262039](https://github.com/democratizedspace/dspace/actions/runs/31632262039), attempt 2,
job `94238380995` succeeded. Its packaged archive SHA-256 is
`2b6528eefd487be8a52db8ea44f118310351976ee32b29fb6115c8081916280a`; this corroborating archive
hash is intentionally documented here rather than added to the release-manifest schema.
...
Do not treat the currently installed 3.0.2 release as evidence for the newly approved 3.0.3
target: the full verifier correctly requires the candidate and live immutable coordinates to agree. The bounded captures above retain only the
