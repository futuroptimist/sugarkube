...
## Production authenticated metrics recovery target

DSPACE production currently runs application **3.0.1** with the historical chart **3.0.2**.
The newly approved recovery target is chart **3.0.3**, while retaining application **3.0.1** and
image `main-1a31a56`. This repository change only onboards that immutable target and its validation
contract: it does not deploy application or chart code, mutate a cluster, or constitute a
promotion. A guarded staging rehearsal and finalized evidence gate require separate authorization
...
permitted differences are the approved chart transition and committed `metrics` and
`serviceMonitor` contract. Retain application 3.0.1, its immutable image digest, and both replicas;
select only chart 3.0.3 at its approved digest, and never use
...
Chart `3.0.3` was published from source revision
`62da11005354e9f9a89c2e58584cdce4c8ec35aa` by upstream PR
[`democratizedspace/dspace#4816`](https://github.com/democratizedspace/dspace/pull/4816) and tag
`chart-v3.0.3`. Publication workflow [run `31632262039`, attempt 2, job
`94238380995`](https://github.com/democratizedspace/dspace/actions/runs/31632262039) completed
successfully. The packaged archive SHA-256 was
`2b6528eefd487be8a52db8ea44f118310351976ee32b29fb6115c8081916280a`; it is provenance for
independent download verification, not the OCI manifest digest and not a release-manifest field.
...
Do not run `dspace-release-verify` against the historical 3.0.2 candidate: the full verifier
...
- `env=prod`: HA production on the production Sugarkube cluster with host `democratized.space` and values `docs/examples/dspace.values.dev.yaml,docs/examples/dspace.values.prod.yaml`.
  The production overlay injects `DSPACE_TOKEN_PLACE_URL=https://token.place` and `DSPACE_TOKEN_PLACE_CHAT_MODEL=llama-3.1-8b-instruct`. Production is pinned to recovery chart `3.0.3` and image `ghcr.io/democratizedspace/dspace:main-1a31a56`; it does not enable metrics or ServiceMonitor settings.
