...
executable SHA-256 values, version `20.20.2`, ARMv7/armhf userspace architecture, realpath, and
`root:root:0755` metadata. Kernel architecture alone is not evidence of userspace compatibility:
an AArch64 kernel may run a 32-bit armhf userspace. Validation reads, but never executes, the
candidate: it requires ELF32 little-endian ARM, exactly one `PT_INTERP` naming
`/lib/ld-linux-armhf.so.3`, an available confined and securely owned executable interpreter, one
non-symlink regular-file identity, the exact digest, and root-owned,
non-group/world-writable coordinate ancestors. Missing, dangling, hard-linked, writable,
wrong-version, wrong-digest, and wrong-architecture candidates fail before Playwright.

The official Node v20.20.2
[`SHASUMS256.txt`](https://nodejs.org/dist/v20.20.2/SHASUMS256.txt) manifest pins the ARMv7 archive at
`f704ce75d9a194c30c378049b516000e49612c2f046ac83c7435eb33ec2926f0`. Independently extracting
only `node-v20.20.2-linux-armv7l/bin/node` produces executable SHA-256
