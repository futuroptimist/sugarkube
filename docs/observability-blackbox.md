...
- `platform/observability/prometheus-blackbox-exporter-values.yaml` keeps three deliberately small modules: generic HTTPS 2xx, bounded JSON health, and static content.
...
- No dev public probes are included because this slice targets staging and production public routes only.
