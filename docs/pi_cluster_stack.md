...
| M3 × 90 mm bolts + M3-0.5 nuts | 4 | Clamp the plates and posts together. Length depends on `levels`, `z_gap_clear`, plate thickness, and how much nut-trap depth you use on the post. Nuts seat into the default M3 nut traps. |
| M3 washers (or equivalent) | 8 | Optional under the bolt heads and/or the bottom nuts if you bypass the traps. |
...
1. **Prep the carriers.** Follow the insert installation guidance in
  [`docs/pi_cluster_carrier.md`](pi_cluster_carrier.md) to seat M2.5 brass inserts (or chase printed
   ...
   for board mounting.
2. **Prep the posts.** Press an M3 nut into the nut trap on the bottom of each corner post (or plan
   to use a washer + nut on the outside if you disabled nut traps).
...
5. **Clamp with bolts.** Insert four long M3 bolts through the top carrier stack-mount holes and
   down through the post bores. Tighten into the captured nuts (or onto bottom nuts with washers) to clamp the
   entire stack.
...
- **Stack clamp:** four long M3 bolts + M3 nuts (captured by default) plus washers if you bypass the traps.
