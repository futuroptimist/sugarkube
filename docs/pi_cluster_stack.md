...
- `openscad` examples (fan components temporarily disabled—see note above):
  ...
  stack-specific STLs organised as `carriers/` (printable stack-mount carriers rendered directly
  from `pi_carrier.scad` with stack mounts enabled), `posts/`, and `preview/` (carrier-level previews
  plus the full assembly). The legacy monorepo bundle `stl-${GITHUB_SHA}` remains available but the
