...
[user-token state access for local execution](https://developer.hashicorp.com/terraform/cloud-docs/workspaces/run/run-environment#user-token),
[user API tokens](https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/users#tokens),
[API token types and expiration](https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/api-tokens),
...
- a short-duration HCP Terraform **user API token** that authenticates the individual authorized
  operator. The associated user must have, for this workspace only, permission to read and write
  state versions, perform the intended local CLI operations, and lock and unlock the workspace when
  this runbook requires it. Set the shortest practical expiration HCP Terraform supports for this
...
For this disposable flow, supply the user API token only through the hidden
`TF_TOKEN_app_terraform_io` environment variable. Do not create or modify `credentials.tfrc.json`,
and do not put the token in command arguments, shell history, logs, plans, state, evidence, or Git.
...
does not revoke the user token. Immediately after the exercise, open the individual operator's HCP
Terraform account settings, select **Tokens**, and explicitly revoke the user API token.

Save only a sanitized summary of authorization, reviewed action counts, resolver outcomes, no-op
results, drift/reconciliation, destruction, empty current state, workspace freeze, and confirmation
that the user token was revoked. Never save tokens, state, saved plans, plan JSON, environment dumps,
complete TXT values, or credential files.
