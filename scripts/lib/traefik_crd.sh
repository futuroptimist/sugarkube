...
traefik_crd::print_report() {
  ...
  if [ "${has_problems}" = false ] && [ "${has_present}" = false ]; then
    ...
    echo "If you intentionally manage Traefik via Helm, you can run 'just traefik-install' to create CRDs."
  elif [ "${has_problems}" = false ]; then
    echo "Next step: run 'just traefik-status' to verify Traefik is running."
    echo "If you intentionally manage Traefik via Helm, you can run 'just traefik-install' to upgrade."
  fi
}
