...
#
# Behavior:
#   - If /etc/rancher/k3s/k3s.yaml does not exist, does nothing and returns 0.
...
kubeconfig::ensure_user_kubeconfig() {
  ...
  if ! kubeconfig::_with_privilege cmp -s /etc/rancher/k3s/k3s.yaml "${kubeconfig_path}" 2>/dev/null; then
