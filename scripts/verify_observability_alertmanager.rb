...
tokenplace_route = environment == "prod" ? children.shift : nil
...
if environment == "prod"
  tokenplace_matchers = ['alertname=~"^(TokenplaceNoHealthyComputeNodes|TokenplaceMetricsTargetDown)$"',
                         'application="tokenplace"', 'environment="prod"',
                         'cluster="sugarkube-prod"', 'severity="critical"']
  fail_closed("token.place route ordering or receiver changed") unless tokenplace_route["receiver"] == TOKENPLACE_RECEIVER
  fail_closed("token.place route matchers are not the exact production allowlist") unless
    tokenplace_route["matchers"].is_a?(Array) && tokenplace_route["matchers"].sort == tokenplace_matchers.sort
  fail_closed("token.place route must contain only receiver and exact matchers") unless
    tokenplace_route.keys.sort == %w[matchers receiver]
end
