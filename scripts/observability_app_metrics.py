...
def validate_inventory(doc):
    ...
    for app, appdoc in apps.items():
        ...
        for env, cfg in environments.items():
            ...
            discovery_labels = {"namespace"} if "namespace" not in mapping else set()
            required_mapping = {
                key: value for key, value in labels.items() if key not in discovery_labels
            }
            if mapping != required_mapping:
                fail(
                    "serviceMonitor.relabelings must map the declared target labels exactly"
                )
