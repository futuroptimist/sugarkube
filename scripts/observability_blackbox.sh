...
normalize_env() {
  ...
  case "${raw}" in
    ...
    "") echo "ERROR: blackbox observability env is required." >&2; exit 2 ;;
...
mutate() {
  ...
  [[ "${ENVIRONMENT}" != staging ]] || kubectl -n "${NAMESPACE}" delete probe "${LEGACY_PROBES[@]}" --ignore-not-found
...
cmd="${1:-}"; shift || true; [[ -n "${cmd}" ]] || { usage; exit 2; }; ENVIRONMENT="$(normalize_env "${1:-}")"; configure_env
