...
normalize_env() {
  ...
  case "${raw}" in
    ...
    "") echo "ERROR: blackbox observability env is required; supported envs: staging, prod." >&2; exit 2 ;;
...
assert_context() {
  ...
  python3 "${ROOT}/scripts/cluster_identity.py" assert --kubeconfig "${kubeconfig}" --env "${ENVIRONMENT}" >/dev/null
}
render_to() {
...
mutate() {
  ...
  require_tools helm kubectl python3 ruby; with_render; print_resolved "<guarded>"; assert_context; preflight; state="$(release_state)"
  ...
  if [[ "${ENVIRONMENT}" == staging ]]; then kubectl -n "${NAMESPACE}" delete probe "${LEGACY_PROBES[@]}" --ignore-not-found; fi
  kubectl apply -f "${PROBE_RENDER}"
}
status() {
  require_tools helm kubectl python3 ruby; with_render; print_resolved "<guarded>"; assert_context
...
validate_resources() {
  ...
  kubectl -n "${NAMESPACE}" get probe -l 'release=kube-prometheus-stack' -o json | python3 "${ROOT}/scripts/verify_blackbox_prometheus.py" --env "${ENVIRONMENT}" --probes
}
prom_get() {
...
verify() { require_tools helm kubectl python3 ruby sleep; with_render; print_resolved "<guarded>"; assert_context; preflight; [[ "$(release_state)" == present ]] || { echo "ERROR: exporter release is absent." >&2; exit 7; }; validate_policy_live; validate_resources; verify_series; }
