...
def parse_sum_line(raw: str):
    line = raw.strip()
    if not line or line.startswith("#"):
        return None, None
    if "(" in line and ")" in line and "=" in line:
        try:
            prefix, digest_part = line.split("=", 1)
            digest = digest_part.strip()
            start = prefix.index("(") + 1
            end = prefix.index(")", start)
            name = prefix[start:end].strip()
            return name, digest
        except ValueError:
            return None, None
...
for raw_line in path.read_text().splitlines():
    name, digest = parse_sum_line(raw_line)
    if not name or not digest:
        continue
    normalized = name.lstrip("./")
    if normalized == target or PurePosixPath(normalized).name == target:
        selected = digest
        break

if not selected:
    sys.stderr.write(f"no checksum entry for {target} in {path}\n")
    sys.exit(1)

path.write_text(f"{selected}  {target}\n")
PY
}
