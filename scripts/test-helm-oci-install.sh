...
spec:
  template:
    spec:
      containers:
        - name: dspace
          image: ghcr.io/democratizedspace/dspace:${tag}
---
apiVersion: v1
kind: Service
metadata:
...
if PATH="${tmp_bin}:${PATH}" HELM_TEST_LOG="${helm_log}" KUBECTL_TEST_LOG="${kubectl_log}" \
    ...
    chart=oci://registry.test/charts/dspace \
    values=docs/examples/dspace.values.dev.yaml \
    version_file=docs/apps/dspace.version tag=v3-deadbee env=prod >"${tmp_bin}/install-mismatch.out" 2>&1; then
...
upgrade_output="$(
    PATH="${tmp_bin}:${PATH}" HELM_TEST_LOG="${helm_log}" KUBECTL_TEST_LOG="${kubectl_log}" \
        ...
        version_file=version_file=docs/apps/dspace.version tag=tag=v3-deadbee env=staging
)"
...
if PATH="${tmp_bin}:${PATH}" HELM_TEST_LOG="${helm_log}" KUBECTL_TEST_LOG="${kubectl_log}" \
    ...
    just helm-oci-upgrade \
    release=dspace namespace=dspace \
    chart=oci://registry.test/charts/dspace version=3.0.1 tag=v3-deadbee env=staging >"${tmp_bin}/upgrade-status-error.out" 2>&1; then
