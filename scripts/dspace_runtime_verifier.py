...
def identity_contract(manifest: dict[str, Any]) -> str:
    """Select the legacy contract only for the complete approved recovery tuple."""
...
def legacy_identity(raw: bytes, revision: str, category: str) -> tuple[str, str, str]:
    """Validate and normalize the bounded legacy build metadata surface."""
    if len(raw) > 1024 * 1024:
        fail(category)
    try:
        value = json.loads(raw)
...
def root_document(raw: bytes, category: str) -> None:
    """Require bounded, non-empty UTF-8 HTML without inferring legacy identity from it."""
