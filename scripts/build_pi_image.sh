...
shopt -s nullglob
if img=$(compgen -G 'deploy/*.img' | head -n1); then
  mv "$img" "${OUTPUT_DIR}/${IMG_NAME}.img"
else
  # pi-gen may emit either .img.zip or plain .zip archives
  zip=$(compgen -G 'deploy/*.img.zip' | head -n1 || true)
  if [ -z "$zip" ]; then
    zip=$(compgen -G 'deploy/*.zip' | head -n1 || true)
  fi
  if [ -n "$zip" ]; then
    unzip -p "$zip" '*.img' > "${OUTPUT_DIR}/${IMG_NAME}.img"
  else
