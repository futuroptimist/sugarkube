...
ENVIRONMENT_RAW="${SUGARKUBE_ENV:-dev}"
ENVIRONMENT="${ENVIRONMENT_RAW}"
if [ "${ENVIRONMENT_RAW}" = "int" ]; then
  log_warn_msg discover_env_alias "env name \"int\" is deprecated; treating as staging" "env=staging"
  ENVIRONMENT="staging"
...
# Security hardening: keep fail-open opt-in for all environments.
...
case "${ENVIRONMENT}" in
  dev)
    if [ -n "${SUGARKUBE_TOKEN_DEV:-}" ]; then
      printf -v TOKEN '%s' "${SUGARKUBE_TOKEN_DEV}"
...
publish_api_service() {
  ...
  if ensure_self_mdns_advertisement server; then
    ...
    if [ "${SAVE_DEBUG_LOGS:-0}" = "1" ]; then
      ...
      # Temporarily unset NO_TERMINATE to use cached results (--terminate flag)
      # instead of waiting indefinitely for network responses
