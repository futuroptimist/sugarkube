...
def validated_runner_path(config: dict) -> Path:
    """Resolve the configured immutable runner without accepting path indirection."""
    root_coordinate = Path(config["runnerRoot"])
    if not root_coordinate.is_absolute() or ".." in root_coordinate.parts:
        raise Invalid("runner path")
    root = Path(os.path.abspath(root_coordinate))
    runner = root / config["runnerRevision"]
    try:
        root_info = root.lstat()
        runner_info = runner.lstat()
        resolved_root = root.resolve(strict=True)
        resolved_runner = runner.resolve(strict=True)
        resolved_runner.relative_to(resolved_root)
    ...
    if (
        ...
        or runner.parent != root
    ):
        raise Invalid("runner path")
    return resolved_runner


def runner_git(runner: Path, *arguments: str) -> subprocess.CompletedProcess[str]:
    """Run read-only Git validation with trust scoped to one validated runner."""
    # The empty value resets any inherited multi-valued safe.directory entries.
    command = [
        "git",
        "-c",
        "safe.directory=",
...
def validate_runner(config: dict) -> Path:
    runner = validated_runner_path(config)
    ...
    if runner_git(runner, "rev-parse", "HEAD").stdout.strip() != config["runnerRevision"]:
        raise Invalid("runner HEAD")
    if runner_git(runner, "status", "--porcelain", "--untracked-files=no").stdout:
        raise Invalid("runner tracked state")
    if (runner / ".git/objects/info/alternates").exists():
        raise Invalid("external object store")
    if runner_git(runner, "rev-parse", "--is-shallow-repository").stdout.strip() != "false":
        raise Invalid("shallow repository")
    runner_git(runner, "fsck", "--full")
    for relative, expected in files.items():
