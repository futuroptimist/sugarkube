...
def validate_node_contract(
    ...
    validate_node_elf_contract(path, contract)
...
def validate_node_interpreter(config: dict, root: Path, contract: dict) -> None:
    """Require the declared loader to resolve safely within the selected root."""
    ...
    try:
        ...
        interpreter_resolved.relative_to(root)
        resolved_info = interpreter_resolved.stat()
    ...
    if (
        ...
        or not os.access(interpreter_resolved, os.X_OK)
        or interpreter_info.st_uid != expected_uid
        or interpreter_info.st_gid != expected_gid
        or resolved_info.st_uid != expected_uid
        or resolved_info.st_gid != expected_gid
        or (not interpreter.is_symlink() and interpreter_info.st_mode & 0o022)
    ...
    validate_node_ancestors(config, root, interpreter_resolved)


def validate_node_elf_contract(path: Path, contract: dict) -> None:
    """Parse ELF headers and require the contract's single dynamic interpreter."""
    try:
        ...
        if phentsize != 32 or phnum < 1 or phoff + phentsize * phnum > len(data):
            raise ValueError
        interpreters = []
        for index in range(phnum):
            offset = phoff + index * phentsize
            p_type, p_offset, p_filesz = struct.unpack_from("<II12xI", data, offset)
        ...
        if (
            ...
            or machine != 40
            or interpreters != [contract["interpreterPath"]]
        ):
            raise ValueError
    except (OSError, KeyError, UnicodeError, struct.error, ValueError):
