...
class CommandRunner:
    ...
    def _run_real(self, args: list[str], timeout: int | float | None) -> CommandResult:
        try:
...
def handle_projects_compose(
    ...
    if shutil.which("docker") or runner.test_mode:
        runner.run(["docker", "compose", "-f", compose_file, "pull"], timeout=None)
        runner.run(["docker", "compose", "-f", compose_file, "up", "-d"], timeout=None)
    else:
...
def handle_cloud_init(
    ...
    runner.run(["cloud-init", "status", "--long"], timeout=None)
    ...
    status = runner.run(["cloud-init", "status", "--wait", "--long"], timeout=None)
