    ...
    *,
    ...
    try:
        ...
        raise TelemetryError(
            f"invalid {description} value: {raw!r} (set {env_var} to a number)"
        ) from exc
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    args.timeout = resolve_timeout(
        explicit=args.timeout,
        env_var="SUGARKUBE_TELEMETRY_TIMEOUT",
        fallback=DEFAULT_TIMEOUT,
        description="timeout",
    )
    args.verifier_timeout = resolve_timeout(
        explicit=args.verifier_timeout,
        env_var="SUGARKUBE_TELEMETRY_VERIFIER_TIMEOUT",
        fallback=DEFAULT_VERIFIER_TIMEOUT,
        description="verifier timeout",
    )
    return args
