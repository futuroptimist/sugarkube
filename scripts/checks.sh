  ...
  if [ -n "${CI:-}" ]; then
    ...
    if [ -n "${GITHUB_BASE_REF:-}" ]; then
      ...
      if ! git rev-parse --verify "$base_ref" >/dev/null 2>&1 && [ -n "$remote_name" ]; then
        git fetch --no-tags \
          "$remote_name" \
          "${GITHUB_BASE_REF}:refs/remotes/${remote_name}/${GITHUB_BASE_REF}" \
          >/dev/null 2>&1 || true
      fi
    ...
    if [ -n "$diff_range" ]; then
      local diff_exit=0
      set +e
      diff_output="$(git diff --name-only "$diff_range" 2>/dev/null)"
      diff_exit=$?
      set -e
      if [ "$diff_exit" -eq 0 ]; then
        ...
        need_log_fallback="true"
      fi
    else
      need_log_fallback="true"
    fi

    if [ "$need_log_fallback" = "true" ]; then
      local log_output
