...
EOF
  ...
  if [ -n "${CI:-}" ]; then
    ...
    local diff_checked="false"
    local diff_found="false"
    if [ -n "$diff_range" ]; then
      if diff_output="$(git diff --name-only "$diff_range" 2>/dev/null)"; then
        diff_checked="true"
    ...
    if [ "$diff_checked" = "false" ] || { [ "$diff_found" = "false" ] && [ "$history_refreshed" = "true" ]; }; then
      local log_output=""
      if log_output="$(git log --name-only --pretty=format: --max-count=50 HEAD 2>/dev/null)"; then
...
maybe_install_kicad() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    local apt_bin add_repo_bin

    apt_bin="$(command -v apt-get || true)"
    if [ -z "$apt_bin" ]; then
      echo "Unable to locate the apt-get executable for KiCad installation" >&2
      return 1
    fi
    ...
    add_repo_bin="$(command -v add-apt-repository || true)"
    if [ -n "$add_repo_bin" ]; then
      if [ "$(id -u)" -ne 0 ]; then
        add_repo_cmd=(sudo -n "$add_repo_bin")
      else
        add_repo_cmd=("$add_repo_bin")
      fi
    else
      add_repo_cmd=()
    fi
    ...
    if ! command -v add-apt-repository >/dev/null 2>&1; then
      ...
      add_repo_bin="$(command -v add-apt-repository || true)"
      if [ -z "$add_repo_bin" ]; then
        echo "Failed to locate the add-apt-repository command after installation" >&2
        return 1
      fi
      if [ "$(id -u)" -ne 0 ]; then
        add_repo_cmd=(sudo -n "$add_repo_bin")
    ...
    if [ ${#add_repo_cmd[@]} -eq 0 ]; then
      echo "add-apt-repository is unavailable for KiCad installation" >&2
      return 1
    fi
