...
# Render active Probe graphs and reject schedules that can exhaust reviewed quotas.
observability-blackbox-quotas env='':
    #!/usr/bin/env bash
    set -euo pipefail
    requested='{{ env }}'
    if [ -n "${requested}" ]; then
      python3 scripts/validate_probe_quotas.py --env "${requested#env=}"
    else
