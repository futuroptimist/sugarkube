...
app-deploy app env='staging' tag='' config='' manifest='' evidence='' staging_evidence='' smoke_runner='' kubeconfig='' staging_config='' staging_kubeconfig='' confirm='':
    ...
    for index in "${!inputs[@]}"; do
      ...
      for prefix in "${prefixes[@]}"; do [[ ${argument} == "${prefix}="* ]] && named=true; done
      ${named} || printf -v "${destinations[index]}" '%s' "${argument}"
    done
    for argument in "${inputs[@]}"; do for prefix_index in "${!prefixes[@]}"; do
      prefix=${prefixes[prefix_index]}
      if [[ ${argument} == "${prefix}="* ]]; then printf -v "${destinations[prefix_index]}" '%s' "${argument#*=}"; break; fi
    done; done
    ...
    if [ -n "${kubeconfig_input}" ]; then export KUBECONFIG="${kubeconfig_input}"; else export KUBECONFIG="${HOME}/.kube/config"; just --justfile "{{ justfile_directory() }}/justfile" kubeconfig-env "${SUGARKUBE_ENV}"; fi
...
dspace-oci-deploy-prod-subdomain tag='' manifest='' evidence='' staging_evidence='' smoke_runner='' kubeconfig='' staging_kubeconfig='' confirm='':
    ...
    for argument in "${inputs[@]}"; do for prefix_index in "${!prefixes[@]}"; do
      if [[ ${argument} == "${prefixes[prefix_index]}="* ]]; then values[prefix_index]=${argument#*=}; break; fi
    done; done
...
# Render the canonical non-Flux kube-prometheus-stack chart for staging or production (read-only).
...
observability-watchdog-secret-install env='':
    @scripts/observability_helm.sh watchdog-secret-install '{{ env }}'

# Check the watchdog Secret/key contract without reading its value.
observability-watchdog-secret-check env='':
    @scripts/observability_helm.sh watchdog-secret-check '{{ env }}'
...
observability-watchdog-drill-status env='':
    @scripts/observability_helm.sh watchdog-drill-status '{{ env }}'
...
# Render the pinned environment-specific blackbox exporter and Probe manifests (read-only).
observability-blackbox-render env='':
    @scripts/observability_blackbox.sh render '{{ env }}'

# Fresh-install the environment-specific blackbox exporter and apply its Probes.
observability-blackbox-install env='':
    @scripts/observability_blackbox.sh install '{{ env }}'

# Upgrade the environment-specific blackbox exporter and apply its Probes.
observability-blackbox-upgrade env='':
    @scripts/observability_blackbox.sh upgrade '{{ env }}'

# Summarize the environment-specific blackbox resources (read-only).
