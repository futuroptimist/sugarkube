...
# Must be run as a normal user (not root); this recipe will ensure $HOME/.kube/config
# exists and is readable by copying /etc/rancher/k3s/k3s.yaml if needed.
traefik-install namespace='kube-system' version='':
    #!/usr/bin/env bash
    set -Eeuo pipefail

    if [ "$(id -u)" -eq 0 ] || [ -n "${SUDO_USER:-}" ]; then
        echo "ERROR: Do not run 'just traefik-install' with sudo. Run it as your normal user (e.g. pi) after configuring kubeconfig." >&2
        exit 1
    fi
    ...
    if [ -r "$HOME/.kube/config" ]; then
        ...
        if [ -f /etc/rancher/k3s/k3s.yaml ]; then
            ...
            echo "ERROR: No readable kubeconfig at $HOME/.kube/config and /etc/rancher/k3s/k3s.yaml not found." >&2
            echo "       Please ensure k3s is installed on this node and kubeconfig is set up." >&2
            exit 1
        fi

        if ! kubectl version --short --client >/dev/null 2>&1; then
            echo "WARNING: kubectl client not found or not working; Helm may still fail if kubectl is missing." >&2
        fi
    fi
