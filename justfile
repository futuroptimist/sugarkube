...
cf-tunnel-install env='dev' token='':
    ...
    config_yaml=$(cat <<EOF
# Token-authenticated tunnel configuration generated by cf-tunnel-install
tunnel: ${tunnel_name}
warp-routing:
  enabled: false
...
EOF
    ...
    kubectl -n cloudflare create configmap cloudflare-tunnel \
        --from-literal=config.yaml="${config_yaml}" \
        --dry-run=client -o yaml | kubectl apply -f -

    kubectl -n cloudflare set env deployment/cloudflare-tunnel \
        --containers cloudflare-tunnel \
        --from=secret/tunnel-token

    kubectl -n cloudflare rollout restart deployment cloudflare-tunnel
