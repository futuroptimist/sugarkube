...
cf-tunnel-install env='dev' token='':
    ...
    if ! helm upgrade --install cloudflare-tunnel cloudflare/cloudflare-tunnel \
        ...
        helm_failed=1
        echo "Helm upgrade/install failed; continuing to apply token-mode patch and diagnostics..." >&2
...
YAML
    ...
    read -r -d '' deployment_patch <<'PATCH' || true
        {"spec":{"template":{"spec":{"containers":[{"name":"cloudflare-tunnel","env":[{"name":"TUNNEL_TOKEN","valueFrom":{"secretKeyRef":{"name":"tunnel-token","key":"token"}}}],"command":["/bin/sh","-c"],"args":["exec cloudflared tunnel --config /etc/cloudflared/config/config.yaml run --token "$TUNNEL_TOKEN""],"volumeMounts":[{"name":"config","mountPath":"/etc/cloudflared/config","readOnly":true}]}],"volumes":[{"name":"config","configMap":{"name":"cloudflare-tunnel","items":[{"key":"config.yaml","path":"config.yaml"}]}}]}}}}
PATCH
    deployment_patch=$(printf '%s' "${deployment_patch}" | sed 's/^    //')
