...
cf-tunnel-install env='dev' token='':
    ...
    if [ "${token_len}" -lt 16 ]; then
        echo "ERROR: CF_TUNNEL_TOKEN appears too short; please copy the full token." >&2
        echo "Recopy the connector token from the Cloudflare dashboard." >&2
        exit 1
    fi

    if ! printf '%s' "${token}" | grep -q '^eyJ'; then
        echo "WARNING: CF_TUNNEL_TOKEN does not look like a JWT (missing 'eyJ' prefix)." >&2
        echo "Confirm it came from the 'cloudflared tunnel --no-autoupdate run --token <TOKEN>' snippet." >&2
    fi
